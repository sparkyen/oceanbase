...
def recursive_entropy(card, values, level, current_indices):
    """递归计算各阶协合作用熵"""
    if level == 0:
        swr = card - sum(values[i] for i in current_indices)
        return calculate_entropy(swr)
    entropy_sum = 0
    for combination in combinations(current_indices, level):
        entropy_sum += (-1) ** (level + 1) * recursive_entropy(card, values, level - 1, combination)
    return entropy_sum

def multi_variable_mutual_info(card, ndvs):
    n = len(ndvs)
    freqs = [card / ndv for ndv in ndvs]
    total_entropy = calculate_entropy(card)
    total_sum = 0
    for level in range(1, n + 1):
        subset_entropy = 0
        for combination in combinations(range(n), level):
            subset_entropy += recursive_entropy(card, freqs, level, combination)
        total_sum += (-1) ** (level + 1) * subset_entropy
    mi = np.exp(total_sum - total_entropy)
    return (1 - mi) * np.prod(ndvs)
...
print(ndvs)
result = multi_variable_mutual_info(card, ndvs[0:1])

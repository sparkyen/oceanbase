  ...
  // ObString file_path;
    ...
    if (OB_ISNULL(full_path_buf = static_cast<char *>(tmp_allocator.alloc(MAX_PATH_SIZE)))) {
      ...
      if(OB_SUCC(ret)) {
        ...
        if(file_size > OB_MAX_LONGTEXT_LENGTH) {
        ...
        } else if (OB_ISNULL(data_buf = static_cast<char *>(tmp_allocator.alloc(data_buf_size)))) {
          ret = OB_ALLOCATE_MEMORY_FAILED;
          ...
          while (!eof && read_size < file_size) {
            ...
            if(OB_FAIL(file_reader.pread(data_buf + read_size, count, offset, this_read_size))){
          ...
          void *lob_buf = tmp_allocator.alloc(data_buf_size + sizeof(ObLobCommon));
          if (OB_ISNULL(lob_buf)) {
            ret = OB_ALLOCATE_MEMORY_FAILED;
            LOG_WARN("alloc buffer failed", K(ret), K(data_buf_size));
          } else {
            ObLobCommon *lob_comm = new(lob_buf)ObLobCommon();
            MEMCPY(lob_comm->buffer_, data_buf, data_buf_size);
